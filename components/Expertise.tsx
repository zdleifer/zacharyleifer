    ...
    <section id="expertise" ref={ref} className="bg-[#0A0A0A] py-28 px-8 md:px-12">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-20 reveal">
          <p className="text-[10px] tracking-[0.35em] uppercase text-[#94A3B8] mb-4">Capabilities</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-0 divide-y md:divide-y-0 md:divide-x divide-white/10">
          {expertise.categories.map((category, i) => (
            ...
            >
              <p className="text-[10px] tracking-[0.3em] uppercase text-[#94A3B8] mb-6">
              ...
              <ul className="space-y-3">
                {category.areas.map((area, j) => (
                  <li key={j} className="text-sm text-white/60 hover:text-white/70 transition-colors flex items-start gap-2">
                    <span aria-hidden="true" className="text-[#475569] mt-1 flex-shrink-0 text-xs">›</span>
                    {area}
                  </li>
                ))}
              </ul>
